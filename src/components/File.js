 ...
 */
async function fileExists(FileMap) {
  try {
    await fs.access(FileMap.absolutePath)
    return true
  } catch(_error) {
    return false
  }
 ...
 *
 * @param {DirMap} DirectoryMap - The directory map to check
 * @returns {Promise<boolean>} Whether the directory exists
 */
async function directoryExists(DirectoryMap) {
  try {
    await fs.access(DirectoryMap.absolutePath)
    return true
  } catch(_error) {
    return false
  }
 ...
 */
function composeFilename(dir, file) {
  assert(
...
function uniformStringArray(arr) {
  return Array.isArray(arr) && arr.every((item, _index, arr) => typeof item === "string")
}
